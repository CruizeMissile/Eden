...
namespace eden
{
    enum class key_t
    {
        ...
        semicolen = 59,
        equals = 61,
        escape = 256, enter, tab, backspace, insert, delete_key, right, left, down, up, pageup, pagedown, home, end,
        capsLock = 280, scrollLock, num_lock, print_screen, pause,
        ...
        key_pad_decimal, key_paddivide, key_pad_multiply, key_pad_subtract, key_pad_Add, key_pad_enter, key_pad_equal,
        left_shift = 340, left_contorl, left_alt, left_super,
    ...
    namespace events
    {
        ...
        {
            key_down_t(const key_t key) : key(key) {}
            key_t key;
        };

        struct key_up_t : public event_t<key_up_t>
        {
            key_up_t(const key_t key) : key(key) {}
            key_t key;
        };

        struct mouse_button_down_t : public event_t<mouse_button_down_t>
        {
            mouse_button_down_t(const mouse_button_t button) : button(button) {}
            mouse_button_t button;
        };

        struct mouse_button_up_t : public event_t<mouse_button_up_t>
        {
            mouse_button_up_t(const mouse_button_t button) : button(button) {}
            mouse_button_t button;
        };

        struct mouse_movement_t : public event_t<mouse_movement_t>
        {
            mouse_movement_t(const u32 x, const u32 y) : x(x), y(y) {}
            u32 x, y;
        };

        struct drag_and_drop_t : public event_t<drag_and_drop_t>
        {
            drag_and_drop_t(const char* file) : file(file) {}
            drag_and_drop_t(const std::string& file_path) : file(file_path) {}
            std::string file;
        };
    }
}
